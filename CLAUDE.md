...
- **ergot dependency**: `wasm/Cargo.toml` path-depends on `../../rust/ergot-wasm/crates/ergot` — a git worktree of the ergot repo pinned to the `wasm-support` branch (all wasm/futures-io ergot work happens on that branch). The main ergot checkout at `../../rust/ergot` can be on any branch.
- **WASM build**: Defined as a Vite Task in `vite.config.ts`, deliberately **not** vp-cached: the ergot path dependency lives outside the workspace root, where vp input globs cannot reach, so a vp cache would serve stale builds. cargo's incremental cache keeps rebuilds fast. `wasm-pack` is a system binary (cargo-installed), not an npm package.
